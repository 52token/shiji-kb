...
因果关系需扩展（新增338条跨章推理），地铁图3185站点标签重叠严重需优化，事件索引和关系需重算。本日完成跨章因果推理和地铁图标签系统重构。

**要解决什么问题？**

跨章因果关系不足，地铁图标签重叠影响可读性，事件索引过时（3186条待重建），关系计算不准（7314条需重算）。

**会影响什么最终交付物？**

因果关系总计7652条（新增338条），地铁图标签防重叠+竖向优化，事件索引和关系重建，地铁图功能拆分提升性能。
